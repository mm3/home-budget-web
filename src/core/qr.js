/**
 * A QR encoder, written out rather than installed: byte mode, the smallest
 * version that fits, Reed-Solomon over GF(256), and the mask chosen by the
 * penalty rules of the specification. Versions 1 to 10 are covered, which is
 * far more than a URL needs.
 *
 * The app draws the result as SVG to share its own address; the build draws the
 * same result as a PNG for the README.
 */
...
export function encodeQr(text, level = 'Q') {
  // TextEncoder rather than Buffer: this module runs in the browser as well.
